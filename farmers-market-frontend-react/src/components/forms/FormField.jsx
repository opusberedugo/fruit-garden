...
export default function FormField({ name, type, placeholder, label, required, value, onChange, className = '' }) {
  ...
  return (
    <div className={`form-field ${className}`}>
      ...
      <input type={type} id={name} name={name} placeholder={placeholder} value={value} className={`w-full px-4 py-3 border ${error ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:outline-none focus:ring-2 ${error ? 'focus:ring-red-500' : 'focus:ring-gray-900'} focus:border-transparent transition-colors`} />
